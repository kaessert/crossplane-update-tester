...
| `UPDATE_TESTER_KUBE_BACKEND` | all cluster commands | Selects the backend for the cluster operations that have been migrated to an in-process client — today the event-list query that backs every update-evidence check, and the resource read that backs every snapshot. Unset (the default) queries the cluster directly through that in-process client, with the event list narrowed to the one resource under test by a server-side selector and the resource read issued as a direct, uncached `Get`. Set to `exec`, every cluster operation instead shells out to the `kubectl` binary (see `KUBECTL` above) — the rollback path if the in-process client ever misbehaves against a cluster this project's own testing did not cover. It rolls back the client only: the `kubectl` event query carries the same server-side selector, so this is not a way to restore the older unfiltered `--all-namespaces` list. |
