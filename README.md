...
| `UPDATE_TESTER_KUBE_BACKEND` | all cluster commands | Selects the backend for the cluster operations that have been migrated to an in-process client — today the event-list query that backs every update-evidence check, the resource read that backs every snapshot, and the merge-patch operations (both the main-body patch every field test and reconcile-nudge issues, and the status-subresource patch the condition-clear step issues) that back every mutation. Unset (the default) queries and patches the cluster directly through that in-process client: the event list narrowed to the one resource under test by a server-side selector, the resource read issued as a direct, uncached `Get`, and each patch issued as a direct RFC-7386 JSON merge patch with the caller's bytes passed through unmodified. Set to `exec`, every cluster operation instead shells out to the `kubectl` binary (see `KUBECTL` above) — the rollback path if the in-process client ever misbehaves against a cluster this project's own testing did not cover. It rolls back the client only: the `kubectl` event query carries the same server-side selector and the `kubectl patch` invocations carry the same merge semantics, so this is not a way to restore any older behaviour. |
