...
| `UPDATE_TESTER_KUBE_BACKEND` | all cluster commands | Selects the backend for the event-list query that backs every update-evidence check. Unset (the default) queries the cluster directly through an in-process client, narrowed to the one resource under test by a server-side selector. Set to `exec`, every cluster operation instead shells out to the `kubectl` binary (see `KUBECTL` above) exactly as earlier releases did — the rollback path if the default ever misbehaves against a cluster this project's own testing did not cover. |
