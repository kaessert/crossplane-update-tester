...
| `UPDATE_TESTER_KUBE_BACKEND` | all cluster commands | Selects the backend for the cluster operations that have been migrated to an in-process client — today the event-list query that backs every update-evidence check, the resource read that backs every snapshot, the manifest-name resolution that turns an example manifest's file path into the live resource identifier every other operation is keyed on, the merge-patch operations (both the main-body patch every field test and reconcile-nudge issues, and the status-subresource patch the condition-clear step issues) that back every mutation, the Ready wait that backs every post-update convergence check, the controller-log read that backs loop detection, and the controller-pod list that backs provider-controller-Deployment resolution and pod-identity reads (the checks that detect a controller restart spoiling an observation window). Unset (the default) queries, patches, waits, reads logs and lists pods against the cluster directly through that in-process client: the event list narrowed to the one resource under test by a server-side selector, the resource read issued as a direct, uncached `Get`, the manifest resolution issued as one direct `Get` per document in the manifest, in document order, failing the whole resolution the moment any document's object does not exist — exactly like a live `kubectl get -f` does, never a decode that never touches the cluster — each patch issued as a direct RFC-7386 JSON merge patch with the caller's bytes passed through unmodified, the wait built from an initial read plus a watch seeded to pick up any transition landing between that read and the watch opening (so a resource already in the target condition returns immediately rather than blocking until timeout), the controller log read as a direct per-pod stream with no line-count limit ever applied, and the controller-pod list issued as a direct, label-selected `Pods` list with no JSONPath template involved. Set to `exec`, every cluster operation instead shells out to the `kubectl` binary (see `KUBECTL` above) — the differential diagnostic that separates "the tool changed" from "the provider changed" on a run whose in-process client misbehaves in a way this project's own testing did not cover. It rolls back every migrated operation identically: the `kubectl` event query carries the same server-side selector, `kubectl get -f` carries the same live, all-or-nothing resolution, the `kubectl patch` invocations carry the same merge semantics, `kubectl wait` itself uses the same list-then-watch approach, `kubectl logs` carries the same `--tail=-1` (no limit), and the pod-list query carries the exact JSONPath templates this project has always issued, so this is not a way to restore any older behaviour. The controller-Deployment RESTART itself (as opposed to resolving which Deployment to restart) is a separate matter: it is issued via `kubectl rollout restart`/`kubectl rollout status` regardless of this variable's value, because those two operations were never migrated to the in-process client. They exist solely so this project's own smoke-test harness (`hack/smoke-test.sh`) has kubectl invocations to assert on, not as a production rollback path — and go away only once that harness itself is rewritten to stop depending on an exec-forced kubectl transcript (a fake API server or equivalent). |
