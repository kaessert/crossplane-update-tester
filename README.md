...
- A working kubeconfig (the `KUBECONFIG` env var, falling back to
  `~/.kube/config` — the same resolution `kubectl` itself applies) configured
  against the cluster under test.
