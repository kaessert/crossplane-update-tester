...
| `UPDATE_TESTER_KUBE_BACKEND` | all cluster commands | Selects the backend for the cluster operations that have been migrated to an in-process client — today the event-list query that backs every update-evidence check, the resource read that backs every snapshot, the manifest-name resolution that turns an example manifest's file path into the live resource identifier every other operation is keyed on, the merge-patch operations (both the main-body patch every field test and reconcile-nudge issues, and the status-subresource patch the condition-clear step issues) that back every mutation, the Ready wait that backs every post-update convergence check, and the controller-log read that backs loop detection. Unset (the default) queries, patches, waits and reads logs against the cluster directly through that in-process client: the event list narrowed to the one resource under test by a server-side selector, the resource read issued as a direct, uncached `Get`, the manifest resolution issued as one direct `Get` per document in the manifest, in document order, failing the whole resolution the moment any document's object does not exist — exactly like a live `kubectl get -f` does, never a decode that never touches the cluster — each patch issued as a direct RFC-7386 JSON merge patch with the caller's bytes passed through unmodified, the wait built from an initial read plus a watch seeded to pick up any transition landing between that read and the watch opening (so a resource already in the target condition returns immediately rather than blocking until timeout), and the controller log read as a direct per-pod stream with no line-count limit ever applied. Set to `exec`, every cluster operation instead shells out to the `kubectl` binary (see `KUBECTL` above) — the rollback path if the in-process client ever misbehaves against a cluster this project's own testing did not cover. It rolls back the client only: the `kubectl` event query carries the same server-side selector, `kubectl get -f` carries the same live, all-or-nothing resolution, the `kubectl patch` invocations carry the same merge semantics, `kubectl wait` itself uses the same list-then-watch approach, and `kubectl logs` carries the same `--tail=-1` (no limit), so this is not a way to restore any older behaviour. |
