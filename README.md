...
| `UPDATE_TESTER_KUBE_BACKEND` | all cluster commands | Selects the backend for the event-list query that backs every update-evidence check. Unset (the default) queries the cluster directly through an in-process client, narrowed to the one resource under test by a server-side selector. Set to `exec`, every cluster operation instead shells out to the `kubectl` binary (see `KUBECTL` above) — the rollback path if the in-process client ever misbehaves against a cluster this project's own testing did not cover. It rolls back the client only: the `kubectl` event query carries the same server-side selector, so this is not a way to restore the older unfiltered `--all-namespaces` list. |
