...
// Create -> Update -> Delete lifecycle for a resource whose backend models
// two object types behind one kind always carries a non-empty
// crossplane.io/external-name annotation, so it never drives the code path
// that resolves identity by search rather than by stored ref. This check
// does, by simulating the scenario that does: it pauses reconciliation,
// strips the external-name annotation, unpauses, and asserts the controller
// recovers to the SAME backend object rather than silently creating a
// duplicate.
//
...
// matches and falls through to a duplicate Create() still derives the
// backend object type from the desired spec (not from how identity was
// searched), so the duplicate's external-name is ALSO correctly prefixed.
// Signal 2 is what actually distinguishes "found the existing object via
// search" from "found nothing, created a second one" — a duplicate always
// emits a second CreatedExternalResource event that a genuine recovery
// never does.
func (r *Runner) RunResolveRecover(m *manifest.Manifest) (*ResolveRecoverResult, error) {
	if m.ExpectExternalNamePrefix == "" {
		return nil, fmt.Errorf("manifest has no %s annotation — nothing to recover-check", manifest.ExpectExternalNamePrefixKey)
	}
