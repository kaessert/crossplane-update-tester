...
// TestMergePatch covers the RFC 7386 simulation in isolation: recursive
// merge into nested objects, explicit-null deletion, and wholesale
// replacement for a non-object patch.
func TestMergePatch(t *testing.T) {
	cases := map[string]struct {
		...
		"OverwritesNamedKeyPreservesOthers": {
			reason: "a patch naming one key overwrites it and leaves every other top-level key untouched",
			target: map[string]interface{}{"a": "1", "b": "2"},
			patch:  map[string]interface{}{"a": "updated"},
			want:   map[string]interface{}{"a": "updated", "b": "2"},
		},
		"ExplicitNullDeletesKey": {
			reason: "RFC 7386 §1: a null value in the patch deletes the corresponding target key",
			target: map[string]interface{}{"a": "1", "b": "2"},
			patch:  map[string]interface{}{"b": nil},
			want:   map[string]interface{}{"a": "1"},
		},
		"RecursesIntoNestedObjects": {
			reason: "a nested object in both target and patch merges recursively rather than replacing wholesale",
			target: map[string]interface{}{
				"outer": map[string]interface{}{"x": "1", "y": "2"},
			},
			patch: map[string]interface{}{
				"outer": map[string]interface{}{"x": "updated"},
			},
			want: map[string]interface{}{
				"outer": map[string]interface{}{"x": "updated", "y": "2"},
			},
		},
		"NonObjectPatchReplacesWholesale": {
			reason: "RFC 7386 §1: a non-object patch (a list here) replaces the target value entirely rather than merging",
			target: map[string]interface{}{"a": []interface{}{"1", "2"}},
			patch:  map[string]interface{}{"a": []interface{}{"3"}},
			want:   map[string]interface{}{"a": []interface{}{"3"}},
		},
		"PatchAddsNewKey": {
			reason: "a patch key absent from target is simply added",
			target: map[string]interface{}{"a": "1"},
			patch:  map[string]interface{}{"b": "2"},
			want:   map[string]interface{}{"a": "1", "b": "2"},
		},
		"NilTargetTreatedAsEmptyObject": {
			reason: "a nil target (a field absent at create time) is treated as {} rather than panicking",
			target: nil,
	...
	for name, tc := range cases {
		t.Run(name, func(t *testing.T) {
			...
			if !reflect.DeepEqual(got, tc.want) {
				t.Errorf("%s: mergePatch() = %#v, want %#v", tc.reason, got, tc.want)
			}
		})
	}
}
